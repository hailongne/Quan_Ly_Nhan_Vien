...
const AssignmentsColumns: React.FC<any> = ({ assignments = [], formatDate, onChangeStatus }) => {
  // Group statuses so columns can include multiple related statuses
  ...
  return (
    <div>
      ...
      <div style={{ maxHeight: 520, overflowY: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 5, width: '100%', boxSizing: 'border-box' }}>

          {/* Column: KPI được giao */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingRight: 12, borderRight: '1px solid rgba(15,23,42,0.06)' }}>
            <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8, textAlign: 'center' }}>KPI được giao</div>
            <div style={{ display: pending.length === 0 ? 'block' : 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, justifyContent: 'center' }}>
              {pending.length === 0 ? (
                <div style={{ color: '#94a3b8' }}>Không có</div>
              ) : (
                pending.map((a: any, i: number) => (
                  <AssignmentCard key={i} assign={a} label={formatDate ? formatDate(a.date) : a.date} />
                ))
              )}
            </div>
          </div>
          ...
          <div
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 12px', borderRight: '1px solid rgba(15,23,42,0.06)' }}
          ...
          >
            ...
            <div style={{ display: inProgress.length === 0 ? 'block' : 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, justifyContent: 'center' }}>
              {inProgress.length === 0 ? (
                <div style={{ color: '#94a3b8' }}>Không có</div>
              ) : (
          ...
          {/* Column: Phê duyệt */}
          <div
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingLeft: 12 }}
          ...
          >
            ...
            <div style={{ display: review.length === 0 ? 'block' : 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, justifyContent: 'center' }}>
              {review.length === 0 ? (
                <div style={{ color: '#94a3b8' }}>Không có</div>
              ) : (
          ...
          {/* Column: Hoàn thành */}
          <div
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingLeft: 12, borderLeft: '1px solid rgba(15,23,42,0.06)' }}
          ...
          >
            ...
            <div style={{ display: completed.length === 0 ? 'block' : 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, justifyContent: 'center' }}>
              {completed.length === 0 ? (
                <div style={{ color: '#94a3b8' }}>Không có</div>
              ) : (
