...
export default function UserKpiDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Chuẩn hoá trạng thái từ DB sang giao diện người dùng (bản đồ nghiêm ngặt)
  ...
  // Cập nhật tối ưu hoá trạng thái cục bộ (so sánh ID như chuỗi để tránh khác kiểu)
  ...
  const handleAssignmentStatusChange = (payload: any, newStatus: UserAssignment['status']) => {
    ...
    const applyUpdate = async (taskId: any) => {
      ...
      // Hoàn tác khi thất bại (dùng cập nhật hàm để tránh đóng băng giá trị)
      try {
        ...
        const dayUpdated = res && res.data ? res.data.dayUpdated : null

        // REFRESH: after a successful status update, refetch assignments for this KPI (for this user)
        // and if the backend did NOT return a dayUpdated payload, refetch the KPI details so UI reflects any automatic day updates.
        try {
          const assignRes = await axios.get(`/api/kpis/${payload.kpiId}/assignments`, { params: { assigned_to: userId } })
          const rows = Array.isArray(assignRes.data) ? assignRes.data : (assignRes.data?.rows ?? [])
          // map rows to UserAssignment shape for this user
          const mapped: UserAssignment[] = rows.map((row: any) => {
            const status = row.status === 'completed'
              ? 'completed'
              : row.status === 'doing'
                ? 'doing'
          ...
          // replace assignments for this KPI in state
          setUserAssignments(prev => {
            // keep other KPI assignments, replace those matching this kpiId
            const others = prev.filter(a => a.kpiId !== payload.kpiId)
            return [...others, ...mapped]
          })
        } catch (err) {
          console.error('[UserKpiDashboard] refetch assignments after status update failed', err)
        }
          ...
          if (dayUpdated) {
            try {
              if (typeof window !== 'undefined' && window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('kpiDayUpdated', { detail: dayUpdated }))
              }
            } catch (e) { /* ignore */ }
          } else {
            const kpiRes = await axios.get(`/api/kpis/${payload.kpiId}`)
            const newKpi = kpiRes.data?.kpi || kpiRes.data
            if (newKpi && newKpi.chain_kpi_id) {
              setKpis(prev => prev.map(k => Number(k.chain_kpi_id) === Number(newKpi.chain_kpi_id) ? newKpi : k))
            }
            try {
              if (typeof window !== 'undefined' && window.dispatchEvent) {
                window.dispatchEvent(new CustomEvent('kpiAssignmentsUpdated', { detail: { chain_kpi_id: payload.kpiId } }))
              }
            } catch (e) { /* ignore */ }
          }
        } catch (err) {
          console.error('[UserKpiDashboard] refetch kpi details after status update failed', err)
        }
      } catch (err) {
        console.error('[UserKpiDashboard] update status error', err)
        // Nếu `taskId` có sẵn thì dùng, nếu không thì cập nhật tối ưu hoá bằng cách đối chiếu cục bộ
        setUserAssignments((prev: UserAssignment[]) => prev.map(a => String(a.taskId) === String(taskId) ? { ...a, status: payload.status } : a))
      }
    }

    // sau đó lấy dữ liệu từ server để xác định `taskId` thật và lưu lại.
    // sau đó lấy dữ liệu từ server để xác định `taskId` thật và lưu lại.
    if (payload.taskId) {
    ...
    (async () => {
      try {
        ...
        // Mỗi album cần ít nhất một file hoặc một link không rỗng, hoặc outputs đã lưu sẵn (khi chỉnh sửa)
        setUserAssignments((prev: UserAssignment[]) => prev.map(a => (a.kpiId === payload.kpiId && a.date === payload.date && Number(a.assignedKpi) === Number(payload.assignedKpi)) ? { ...a, status: payload.status } : a))
      }
    })()
  }
              ...
              <div key={kpi.chain_kpi_id} style={{
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div>
                      <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 2, lineHeight: 1.15 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 11, opacity: 0.9 }}>Tổng KPI phòng</div>
                      <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1 }}>{kpi.total_kpi}</div>
                    </div>
                  </div>
                </div>
