...
import useCloseOnEscAndOutside from '../../../hooks/useCloseOnEscAndOutside'
...
const UploadModal: React.FC<{ onClose?: () => void; onSaved?: () => void; maxResults?: number; taskId?: number; chainKpiId?: number; kpiNames?: string[]; mode?: 'upload' | 'view'; existing?: { albums: ExistingAlbum[]; items: ExistingItem[]; links?: ExistingLink[] } | null; disableActions?: boolean }> = ({ onClose, onSaved, maxResults = 0, taskId, chainKpiId, kpiNames, mode = 'upload', existing, disableActions = false }) => {
  ...
  useCloseOnEscAndOutside(containerRef, onClose)
  const isLeaderContext = React.useMemo(() => {
    try {
      return typeof window !== 'undefined' && String(window.location.pathname).includes('/leader')
    } catch (e) {
      return false
    }
  }, [])
  const hideActions = disableActions || isLeaderContext
  ...
  const updateAlbumName = (id: string, name: string) => {
    setResults(prev => prev.map(r => r.id === id ? { ...r, albumName: name } : r))
  }
  ...
  const formValid = React.useMemo(() => {
    ...
    for (let idx = 0; idx < list.length; idx++) {
      ...
      const albumName = String((result as any).albumName ?? (result.album_name ?? '')).trim()
  ...
  const handleSave = async () => {
    ...
    if (!formValid) {
      notify.error('Vui lòng điền tên album cho tất cả album và upload kết quả')
      return
    }
    setSaving(true)
    try {
      const payload = {
        albums: results.map((r, idx) => {
          ...
          return {
            ...
            albumName: (r as any).albumName || null,
            links
          }
        })
      }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 12 }}>
      ...
      <div ref={containerRef} style={{ position: 'relative', background: 'linear-gradient(180deg,#ffffff 0%,#f8fafc 100%)', borderRadius: 16, padding: 22, width: 760, maxWidth: '94vw', boxShadow: '0 24px 60px rgba(2,6,23,0.35)', maxHeight: '88vh', overflow: 'auto', border: '1px solid #e2e8f0' }}>
            ...
            <div key={result.id ?? result.album_id ?? idx} style={{ border: '1px solid #e2e8f0', borderRadius: 14, padding: 14, position: 'relative', background: '#ffffff', boxShadow: '0 6px 18px rgba(15,23,42,0.06)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#696c72' }}>Album: </div>
                  {modeState === 'view' ? (
                    <div style={{ fontSize: 18, color: '#475569', fontWeight: 700 }}>{(result as any).albumName || (result.album_name || '')}</div>
                  ) : (
                    <input
                      value={(result as any).albumName || ''}
                      onChange={(e) => updateAlbumName(result.id ?? String(idx), e.target.value)}
                      placeholder="Tên album (tùy chọn)"
                      style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, maxWidth: 240 }}
                    />
                  )}
                </div>
              </div>
              ...
              {(modeState === 'view' || modeState === 'edit') && existingState && Array.isArray(existingState.links) && (
                <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {existingState.links.filter(l => l.album_id === (result.album_id ?? result.albumId)).map((l) => (
                    ...
                    >
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#dbe6ff', textDecoration: 'underline' }}>Link kết quả công việc</span>
                      {modeState === 'edit' && (
                        <button
