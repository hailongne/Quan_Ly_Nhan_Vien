...
export default function ManageStaffKpi() {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      padding: '12px',
                      boxShadow: active ? '0 6px 18px rgba(13,110,253,0.08)' : '0 1px 6px rgba(16,24,40,0.04)',
                      border: active ? '2px solid #1d9dd8' : '1px solid #eef2f7',
                      cursor: 'pointer',
                      display: 'flex',
                  ...
                  >
                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', width: '100%' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#0b3b66', marginBottom: 6, lineHeight: '1.1' }}>{title}</div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', marginLeft: 8 }}>
                        <div style={{ background: '#eef9ff', color: '#0369a1', padding: '10px 14px', borderRadius: 8, fontWeight: 800, fontSize: 16, minWidth: 68, textAlign: 'center' }}>{total} KPI</div>
                      </div>
                    </div>
                  </button>
                )
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {rows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ borderRadius: 10, padding: '12px 12px', border: selectedUserId === r.userId ? '2px solid #1d9dd8' : (hoveredRow === r.userId ? '2px solid #cfeeff' : '1px solid #e6eef8'), display: 'flex', gap: 10, alignItems: 'center', background: selectedUserId === r.userId ? '#f3fbff' : (hoveredRow === r.userId ? '#fbfdff' : '#fff'), width: '100%', boxShadow: selectedUserId === r.userId ? '0 12px 34px rgba(13,110,253,0.06)' : (hoveredRow === r.userId ? '0 10px 30px rgba(2,6,23,0.06)' : '0 6px 18px rgba(2,6,23,0.04)'), transition: 'all 160ms ease' }}>
                        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                          <div style={{ fontWeight: 900, color: '#0b3b66', fontSize: 14, lineHeight: '1.05' }}>{r.name}</div>
                        ...
                        {rows.map((r) => (
                          <div key={`${pageIdx}-${r.userId}`} style={{ display: 'grid', gridTemplateColumns: `repeat(7, 1fr)`, gap: 8, minHeight: ROW_HEIGHT, alignItems: 'stretch', transition: 'transform 220ms ease, opacity 200ms ease', opacity: selectedUserId && Number(r.userId) !== Number(selectedUserId) ? 0.12 : 1, transform: selectedUserId && Number(r.userId) !== Number(selectedUserId) ? 'scaleY(0.98)' : 'none', pointerEvents: selectedUserId && Number(r.userId) !== Number(selectedUserId) ? 'none' : 'auto', marginBottom: ROW_GAP }}>
                            {weekDates.map((d) => {
                              ...
                              const primaryColor = isToday ? '#06b6d4' : (isReview ? '#be185d' : (isCompleted ? '#16a34a' : '#7c3aed'))
                              return (
                                <div
                                  key={d}
                                  onMouseEnter={() => setHoveredRow(r.userId)}
                                  ...
                                  style={{
                                    ...
                                    boxShadow: isToday ? '0 2px 20px rgba(6, 181, 212, 0.77)' : 'none',
                                ...
                                >
                                  <div>
                                    <div style={{ fontSize: 11, color: isZero ? '#9ca3af' : '#0b3b66' }}>
                                      <span>{weekday}, </span>
                                      <span style={{ fontWeight: 800, color: isZero ? '#9ca3af' : (isToday ? primaryColor : '#0b3b66') }}>{formatDate(d)}</span>
                                    </div>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <div style={{ fontSize: 30, fontWeight: 800, color: isZero ? '#9ca3af' : (isToday ? primaryColor : '#0b3b66') }}>{count}</div>
                                    <div style={{ marginLeft: 6, fontSize: 12, fontWeight: 700, color: isZero ? '#9ca3af' : '#0b3b66' }}>KPI</div>
                                  </div>
                        ...
                        {weekDates.map((d) => {
                          ...
                          return (
                            <div key={`footer-${d}`} style={{ padding: '6px 6px', borderRadius: 10, background: isZero ? '#f8fafc' : '#ffffff', border: isZero ? '1px solid #e6eaf0' : '1px solid #e6eef8', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxShadow: isZero ? 'none' : '0 6px 18px rgba(2,6,23,0.04)', opacity: isZero ? 0.85 : 1 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                                <div style={{ fontSize: 22, fontWeight: 900, color: isZero ? '#9ca3af' : '#0b3b66' }}>{total}</div>
                                <div style={{ fontSize: 11, fontWeight: 800, color: isZero ? '#9ca3af' : '#0b3b66' }}>KPI</div>
                              </div>
                            </div>
                          )
                        })}
