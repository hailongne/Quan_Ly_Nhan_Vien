...
export default function TransferredKpiCardList({ transferredOnly = true }: { transferredOnly?: boolean }) {
  const { user } = useAuth();
  ...
  function AssignButton({ chain_kpi_id, onAssign }: { chain_kpi_id: number; onAssign: (id: number) => void }){
    ...
    const base: React.CSSProperties = {
      ...
      background: hover ? '#db2777' : '#fff',
      color: hover ? '#fff' : '#db2777',
      cursor: 'pointer',
      fontWeight: 700,
      fontSize: 13,
      boxShadow: hover ? '0 6px 14px rgba(219,39,119,0.16)' : undefined,
      ...
      <div style={wrapStyle} ref={ref}>
        ...
        {open && (
          <div role="menu" style={menuStyle}>
            <button role="menuitem" onMouseEnter={()=>setHover(null)} onMouseLeave={()=>setHover(null)} onClick={()=>{ onChange(null); setOpenDropdown(null); }} style={{ ...itemStyle, background: value === null ? '#fdf2f8' : 'transparent', color: value === null ? '#be185d' : '#0f1723' }}>
              Tất cả
            </button>
            {opts.map((opt)=> (
              <button key={opt} role="menuitem" onMouseEnter={()=>setHover(opt as number)} onMouseLeave={()=>setHover(null)} onClick={()=>{ onChange(opt as number); setOpenDropdown(null); }} style={{ ...itemStyle, background: hover === (opt as number) ? '#fdf2f8' : (opt === value ? '#fce7f3' : 'transparent'), color: opt === value ? '#be185d' : '#0f1723' }}>
  ...
  return (
    <div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
            <div style={{ marginBottom: 12, fontWeight: 700, color: '#9d174d' }}>Phòng ban của bạn: <span style={{ fontWeight: 600, color: '#be185d' }}>{deptName ?? '—'}</span></div>
      ...
      {filteredList.map((k) => {
        ...
        <div
          ...
          role="button"
          tabIndex={0}
          onClick={() => window.dispatchEvent(new CustomEvent('assignTransferredKpi', { detail: { chain_kpi_id: k.chain_kpi_id } }))}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); window.dispatchEvent(new CustomEvent('assignTransferredKpi', { detail: { chain_kpi_id: k.chain_kpi_id } })); } }}
          style={{ cursor: 'pointer', background: '#fff', border: '1px solid #fbcfe8', borderRadius: 10, padding: 14 }}
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
            <div style={{ fontWeight: 800, color: '#9d174d' }}>{Array.isArray(k.kpi_name) && k.kpi_name.length ? k.kpi_name.join(' • ') : (k.description || 'KPI')}</div>
          </div>
          ...
          <div style={{ marginTop: 8, fontSize: 13, color: '#9d174d' }}>
            Phòng ban phụ trách: <strong>{cardDeptName ?? '—'}</strong>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 10 }}>
            <div style={{ fontSize: 13, color: '#9d174d' }}>Tổng: <strong>{k.total_kpi ?? '—'} KPI</strong></div>
            <div>
              <AssignButton chain_kpi_id={k.chain_kpi_id} onAssign={(id) => window.dispatchEvent(new CustomEvent('assignTransferredKpi', { detail: { chain_kpi_id: id } }))} />
            </div>
          </div>
        </div>
      )})}
