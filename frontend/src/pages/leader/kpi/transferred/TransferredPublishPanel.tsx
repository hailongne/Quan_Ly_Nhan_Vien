...
export default function TransferredPublishPanel({ initialId = null }: Props) {
  const { user } = useAuth()
  ...
  const storedId = (() => { try { const v = localStorage.getItem('leader.transferred.publish.kpiId'); return v ? Number(v) : null } catch (_) { return null } })()
  
  const weekBoxBase: React.CSSProperties = { borderRadius: 6, padding: 10, marginBottom: 10, position: 'relative', background: '#ffffff', border: '1px solid #fbcfe8', paddingLeft: 20 }
  const leftBarStyle: React.CSSProperties = { position: 'absolute', left: 0, top: 0, bottom: 0, width: 6, background: 'linear-gradient(to right,#f9a8d4,transparent)', pointerEvents: 'none' }
  const hrTopStyle: React.CSSProperties = { position: 'absolute', left: 0, right: 0, top: 0, height: 1, background: 'linear-gradient(to right,#f9a8d4,transparent)', opacity: 0.9, pointerEvents: 'none' }
  const hrBottomStyle: React.CSSProperties = { position: 'absolute', left: 0, right: 0, bottom: 0, height: 1, background: 'linear-gradient(to right,#f9a8d4,transparent)', opacity: 0.9, pointerEvents: 'none' }
  ...
  const weekRangeStyle: React.CSSProperties = { fontSize: 12, color: '#be185d' }
  const weekMetaStyle: React.CSSProperties = { display: 'flex', fontSize: 12, alignItems: 'center', justifyContent: 'space-between', gap: 8, color: '#be185d', marginBottom: 6 }
  ...
  const getDayStyle = (completed: boolean, noTarget: boolean): React.CSSProperties => {
    ...
    if (noTarget) return { ...dayBaseSized, background: '#fdf2f8', color: '#ec4899', borderColor: '#fbcfe8' };
    return { ...dayBaseSized, background: '#fdf2f8', color: '#9d174d', borderColor: '#f9a8d4' };
  }
  ...
  function AssignButton({ onAssign, disabled, children }: { onAssign: () => void, disabled?: boolean, children?: React.ReactNode }){
    ...
    const base: React.CSSProperties = {
      ...
      background: disabled ? '#ffffff' : (hover ? 'linear-gradient(90deg,#fff1f7,#ffe4ef)' : '#fff'),
      color: disabled ? '#94a3b8' : '#be185d',
      cursor: disabled ? 'not-allowed' : 'pointer',
      fontWeight: 600,
      fontSize: 13,
      boxShadow: !disabled && hover ? '0 6px 18px rgba(190,24,93,0.12)' : 'none',
  ...
  useEffect(() => {
    ...
    async function load(id?: number | null) {
      ...
      try {
        ...
        if (!isTransferredKpi(data)) {
          try { localStorage.removeItem('leader.transferred.publish.kpiId') } catch (_) {}
          setKpi(null)
          setWeeks([])
          setEmptyNotice('Tab Giao KPI của KPI điều phối chỉ hiển thị KPI điều phối.')
          try {
        ...
        // reset refs
        weekRefs.current = []
        try { localStorage.setItem('leader.transferred.publish.kpiId', String(id)) } catch (_) { /* ignore */ }
      } catch (err: any) {
        if (cancelled) return
    ...
    const stored = (() => { try { const v = localStorage.getItem('leader.transferred.publish.kpiId'); return v ? Number(v) : null } catch (_) { return null } })()
  ...
  const selectKpi = async (id: number | string) => {
    try { localStorage.setItem('leader.transferred.publish.kpiId', String(id)) } catch (_) {}
    try {
      setLoading(true)
      const res = await axios.get(`/api/kpis/${id}`)
      const data = res.data?.kpi ?? res.data
  ...
  if (!initialId && !storedId) return (
    <div style={{ padding: 16, background: '#fff', borderRadius: 8 }}>
      <div>
        ...
        {!loadingKpis && emptyNotice && <div style={{ color: '#be185d', marginBottom: 8 }}>{emptyNotice}</div>}
        ...
        {!loadingKpis && Array.isArray(availableKpis) && availableKpis.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 12, marginTop: 8 }}>
            {availableKpis.map((kp: any) => {
              ...
              return (
                <div key={kp.id ?? kp._id ?? kp.chain_kpi_id} onClick={() => selectKpi(kp.id ?? kp._id ?? kp.chain_kpi_id)} style={{ padding: 12, borderRadius: 8, background: '#fff', border: '1px solid #fbcfe8', cursor: 'pointer', boxShadow: '0 6px 18px rgba(15,23,42,0.04)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ fontWeight: 700, color: '#9d174d' }}>{title}</div>
                  ...
                  <div style={{ marginTop: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ color: '#be185d', fontWeight: 700 }}>{kp.total_kpi ?? '—'} KPI</div>
                    <button style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: '#db2777', color: '#fff', cursor: 'pointer' }}>Chọn</button>
                  </div>
                </div>
              )
            })}
      ...
      <div style={{ background: '#fff', padding: 12, borderRadius: 8 }}>
        <div style={headerAreaStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ color: '#9d174d', fontWeight: 700 }}>Tổng KPI: {kpi.total_kpi ?? '—'}</div>
          </div>
        </div>
